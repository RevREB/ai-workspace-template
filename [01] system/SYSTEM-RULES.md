...
- **Host-isolated at launch — `HOME` is sealed for the AI CLIs, not for you.**
  Each roster CLI gets a shim in `bin/` (generated by `devbox run provision`,
  first on `PATH`) that runs it through `scripts/sealed-exec.sh`. That wrapper
  points `$HOME` at `.aihome/home` and redirects git/npm/XDG config there, so
  the agent *and every process it spawns* land inside the workspace — including
  tools that hardcode `~/.foo` and tools not installed yet. Nothing is read from
  or written to the host: not `~/.claude`, not `~/.config`, not `~/.gitconfig`.
  ...
  - **Why not seal the whole shell.** Putting `HOME` in `devbox.json`'s `env`
    works, and is wrong: direnv applies it on `cd`, so merely standing in the
    directory strips the human's own git identity, credentials, and `~/.zshrc`.
    Isolation is for the agents; the devbox shell is for the human and supplies
    only the *toolchain*. `doctor` treats a sealed ambient shell as a fault.
  - **Why `HOME` and not more per-tool variables.** Redirect-by-variable only
    ...
    scoped to the launch, it costs the human nothing.
  - **Shims, not discipline.** `bin/` precedes `.aihome/npm/bin` on `PATH`, so
    typing `claude` runs it sealed without anyone remembering a wrapper. A
    roster CLI without a shim would run unsealed, so `doctor` fails on one.
    `bin/sealed <cmd>` runs anything else the way an agent would see it.
...
- **Bridge the credential, never exempt the program.** A sealed home has no
  ...
  - **Bridging is automatic**, applied on every sealed launch by
