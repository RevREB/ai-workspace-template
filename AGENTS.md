...
**The AI CLIs are sealed; your shell is not.** Isolation is applied at launch by
`bin/` shims (generated by `devbox run provision`) which run each CLI through
`scripts/sealed-exec.sh`. That wrapper points `$HOME` at `.aihome/home` and
redirects git/npm/XDG config there, so the agent and everything it spawns read
and write only inside the workspace — including tools that hardcode `~/.foo`.

It is deliberately NOT in `devbox.json`'s env: that would seal the ambient shell,
so merely `cd`-ing into the directory (direnv) would strip the human's own git
identity, credentials and `~/.zshrc`. The devbox shell provides the toolchain;
the shims provide the isolation. Consequences:
...
- A sealed home has no credentials. They are bridged back **automatically on
  every sealed launch** per `[01] system/bridges.json` — git identity, `gh` auth, and
...
- `bin/sealed <cmd>` runs any command the way an agent sees it (`bin/sealed git
  status`, `bin/sealed bash`). Useful for checking what the agent can actually
  authenticate with, which is not what your shell can.
