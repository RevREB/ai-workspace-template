...
The AI CLIs launch through sealing shims in `bin/` (generated by
`devbox run provision`, ahead of everything on `PATH`). A shim runs the CLI via
`scripts/sealed-exec.sh`, which points `$HOME` at `.aihome/home` and redirects
git/npm/XDG config there — so the agent, and every process it spawns, stays
inside the workspace, including tools that hardcode `~/.foo`. Because the
workspace ships its own `AGENTS.md`, AGENTS.md-aware CLIs also won't fall back to
host conventions. Net: what the agent sees is defined entirely by this repo.

**Your own shell is not sealed.** Isolation belongs to the AI CLIs, not to a
human standing in the directory — sealing through `devbox.json`'s env would mean
merely `cd`-ing in (direnv) stripped your own git identity, credentials and
`~/.zshrc`. The devbox shell gives you the *toolchain*; the shims give the agents
the *isolation*. `devbox run doctor` treats a sealed ambient shell as a fault.
Use `bin/sealed <cmd>` to run anything the way an agent would see it.

A sealed home has no credentials, so they are bridged back **automatically on
every sealed launch**, per the policy in `[01] system/bridges.json` (git
identity, `gh` auth, `~/.ssh`). Nothing to run, nothing to remember. The rule is
...
`devbox run doctor` probes the wrapper by *running* it rather than trusting env
vars, and checks containment rather than literal paths so it behaves the same on
macOS and Linux.
...
2. `cd` in, `direnv allow` (or `devbox shell`). Your shell stays unsealed.
