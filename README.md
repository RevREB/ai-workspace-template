...
`devbox.json` points `$HOME` itself at `.aihome/home`, so everything launched in
the shell stays inside the workspace — including tools that hardcode `~/.foo`
and tools not installed yet. `AI_HOME`, `XDG_*`, and per-CLI `*_CONFIG_DIR` back
it up explicitly. Because the workspace ships its own `AGENTS.md`,
...
shell entry**, per the policy in `[01] system/bridges.json` (git identity, `gh`
auth, `~/.ssh`). You never run a command to unlock the workspace — a workspace
you must remember to unlock is the same fight in a different shape. The rule is
**bridge the credential, never exempt the program**: an exemption is inherited by
everything that program spawns, so an "exempt" `git` is a door to the whole host,
while a bridged credential leaks exactly one thing. Credentials are *capability*;
host config is *context* — only the former is bridged, which is why a stale host
`~/.gitconfig` still can't reach in and make the next agent wrong.

`devbox run doctor` probes `git` and `npm` directly for leaks rather than
...
2. `cd` in, `direnv allow` (or `devbox shell`) — credentials bridge themselves here.
